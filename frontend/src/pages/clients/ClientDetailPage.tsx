...
function getClientDisplayName(client?: Client | null) {
    if (!client) return "-";
    return client.type === "institution"
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setError(
                err?.data?.message ??
                err?.data?.error ??
                t("clients.detail.loadFailed", "Failed to load client details.")
            );
        } finally {
            setLoading(false);
        }
    }, [canViewClients, navigate, numericId, slug, t]);

    useEffect(() => {
        load();
    }, [load]);

    const displayName = useMemo(() => getClientDisplayName(client), [client]);
        ...
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
                ...
                <h1 className="text-xl font-bold text-gray-900 mb-1">
                    {t("clients.forbidden.title", "Access denied")}
                </h1>
            ...
            <div className="px-0 py-2">
                <nav className="lims-breadcrumb">
                    ...
                    <span className="lims-breadcrumb-current">
                        {t("clients.detail.breadcrumbCurrent", "Client detail")}
                    </span>
                </nav>
            </div>
                    ...
                    <div className="space-y-8">
                        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
                            <div className="w-full lg:w-1/3 bg-gray-50/50 rounded-2xl p-5 border border-gray-100 flex flex-col items-center text-center">
                                ...
                                <h2 className="text-xl font-bold text-gray-900 wrap-break-word w-full px-2">
                                    {displayName}
                                </h2>
                                ...
                                <div className="w-full space-y-3 text-sm text-left bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                                    {client.type === "individual" && client.phone && (
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="text-[10px] text-gray-400 uppercase tracking-wider">
                                                    {t("clients.detail.labels.phone", "Phone")}
                                                </div>
                                    ...
                                    {client.email && (
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="text-[10px] text-gray-400 uppercase tracking-wider">
                                                    {client.type === "institution"
                                                        ? t("clients.detail.labels.accountEmail", "Account email")
                                    ...
                                    {client.type === "individual" && (client.address_domicile || client.address_ktp) && (
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="min-w-0 flex-1 space-y-2">
                                                <div className="text-[10px] text-gray-400 uppercase tracking-wider">
                                                    {t("clients.detail.labels.address", "Address")}
                                                </div>
                                                {client.address_domicile && (
                                                    <div>
                                                        <span className="text-[10px] text-gray-500 bg-gray-100 px-1.5 rounded mr-1">
                                                            {t("clients.detail.labels.domicile", "Domicile")}
                                                        </span>
                                                ...
                                                {client.address_ktp && (
                                                    <div>
                                                        <span className="text-[10px] text-gray-500 bg-gray-100 px-1.5 rounded mr-1">
                                                            {t("clients.detail.labels.ktp", "KTP")}
                                                        </span>
                                    ...
                                    {client.type === "institution" && client.institution_address && (
                                        <div className="flex items-start gap-3">
                                            <div className="h-8 w-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-500 shrink-0">
                                                <MapPin className="h-4 w-4" />
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <div className="text-[10px] text-gray-400 uppercase tracking-wider">
                                                    {t("clients.detail.labels.institutionAddress", "Institution address")}
                                                </div>
                                                <div className="font-medium text-gray-900">{client.institution_address}</div>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="flex-1 w-full space-y-6">
                                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                                        ...
                                        <div>
                                            <div className="lims-detail-label">
                                                {t("clients.detail.labels.clientId", "Client ID")}
                                            </div>
                                        ...
                                        <div>
                                            <div className="lims-detail-label inline-flex items-center gap-1">
                                                ...
                                                {t("clients.detail.labels.createdAt", "Created at")}
                                            </div>
                                            <div className="lims-detail-value">{formatDate(client.created_at)}</div>
                                        </div>
                                        <div>
                                            <div className="lims-detail-label">
                                                {t("clients.detail.labels.updatedAt", "Last updated")}
                                            </div>
                                    ...
                                    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                                            <div>
                                                ...
                                                <div className="lims-detail-value font-mono">
                                                    {client.national_id || "-"}
                                                </div>
                                            </div>
                                            <div>
                                    ...
                                    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                                            <div className="md:col-span-2">
                                                <div className="lims-detail-label">
                                                    {t("clients.detail.labels.institutionName", "Institution name")}
                                                </div>
                                                <div className="lims-detail-value font-semibold">
                                                    {client.institution_name || client.name || "-"}
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                <div className="lims-detail-label">
                                                    {t("clients.detail.labels.contactPersonPhone", "Contact person phone")}
                                                </div>
                                                <div className="lims-detail-value">
                                            ...
                                            <div>
                                                <div className="lims-detail-label">
                                                    {t("clients.detail.labels.contactPersonEmail", "Contact person email")}
                                                </div>
